	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(uri!=null) {
			if(uri.endsWith("addVehicle.do")) {
				...
				//auto generated don't give customer id as parameter(detached entity)
				...
				//use CustomerDao to check whether user is a valid user or not
				//we have assumed that user is valid
				//true creates a session if session doesn't exist else it fetches existing session
				//false=>fetch existing session if it exists. Doesn't care
				...
				HttpSession ses= request.getSession(false); //don't create session if present get it
